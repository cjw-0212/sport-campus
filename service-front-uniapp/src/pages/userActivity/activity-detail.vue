<template>
  ...
  <up-modal :show="show4" content='此操作将删除与此活动有关的全部信息，是否确认' @confirm="deleteActivity" @cancel="show4=false"
            :showCancelButton="true" contentTextAlign="center" confirmColor="#fa3534"></up-modal>
  <view style="margin: 20rpx;padding: 15rpx" v-show="publishUserId===userStore.user.id">
    <up-sticky offset-top="750rpx">
      <up-row customStyle="margin-bottom: 10px">
        <up-col span="6">
          <up-button type="primary" :text="isCard==1?'已开启打卡':'开启打卡'" style="margin: 20rpx;padding: 15rpx"
                     shape="circle" :disabled="isCard==1||isFinish==1"
      ...
      <up-row v-if="isFinish==1">
        <up-col span="4"></up-col>
        <up-col span="4">
          <up-button type="error" text="删除" style="margin: 20rpx;padding: 15rpx"
                     shape="circle" @click="show4=true"></up-button>
        </up-col>
        <up-col span="4"></up-col>
      </up-row>
    </up-sticky>
...
const deleteActivity = async () => {
  show4.value = false
  const res = await requestDeleteActivity(activityId.value)
  if (res.code === 1) {
    uToastRef.value.show({
      type: 'success',
      message: "删除成功",
